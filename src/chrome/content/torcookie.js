...
var cookiesTreeView = {
  ...
  cycleHeader(column, aElt) {},
  getRowProperties(row, column, prop) {},
  getColumnProperties(column, columnElement, prop) {},
  getCellProperties(row, column, prop) {},
 };
...
function initDialog() {
  ...
  selector = Cc["@torproject.org/cookie-jar-selector;1"]
                    .getService(Ci.nsISupports)
                    .wrappedJSObject;
  // init cookie manager
  cookiemanager = Cc["@mozilla.org/cookiemanager;1"].getService();
    cookiemanager = cookiemanager.QueryInterface(Ci.nsICookieManager);
...
function ProtectInTree(tree, view, table, protButton, unprotButton, removeButton) {
  var selections = getTreeSelections(tree);
...
function UnProtectInTree(tree, view, table, protButton, unprotButton, removeButton) {
  var selections = getTreeSelections(tree);
  for (var s = selections.length - 1; s >= 0; s--) {
    var i = selections[s];
...
function DeleteSelectedItemFromTree(tree, view, table, deletedTable, removeButton, removeAllButton, protButton, unprotButton) {
  var selections = getTreeSelections(tree);
...
function DeleteAllFromTree(tree, view, table, deletedTable, removeButton, removeAllButton, protButton, unprotButton) {

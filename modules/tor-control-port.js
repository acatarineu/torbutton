...
io.asyncSocketStreams = function(ipcFile, host, port) {
  ...
  let outputStream = socketTransport
      .openOutputStream(UNBUFFERED, 1, 1)
      .QueryInterface(Ci.nsIAsyncOutputStream),
    // Open unbuffered asynchronous inputStream.
    inputStream = socketTransport
      .openInputStream(UNBUFFERED, 1, 1)
      ...
      inputStream.asyncWait(
        {
          onInputStreamReady: stream => {
            try {
              let chunk = scriptableInputStream.read(
...
io.matchRepliesToCommands = function(asyncSend, dispatcher) {
  ...
  dispatcher.addCallback(/^[245]\d\d/, function(message) {
    ...
    if (message.match(/^2/) && replyCallback) {
  ...
  return command =>
    new Promise(function(replyCallback, errorCallback) {
...
tor.controller = function(ipcFile, host, port, password, onError) {
  ...
  return {
    ...
    onionAuthAdd: (hsAddress, b64PrivateKey, nickname, isPermanent) =>
      onionAuth.add(socket, hsAddress, b64PrivateKey, nickname, isPermanent),
...
var controller = function(onError) {
  ...
  tor.controllerCache[dest] = tor.controller(

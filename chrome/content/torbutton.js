 ...
 */
  ...
  let startupObs = Cc["@torproject.org/startup-observer;1"].getService(
    Ci.nsISupports
  ).wrappedJSObject;
  const {
    ...
    m_tb_control_pass,
    m_tb_control_desc,
  } = startupObs.TorGetControlParams();
  ...
  var torbutton_unique_pref_observer = {
    register() {
      ...
      m_tb_prefs.addObserver("plugin.disable", this);
    ...
    observe(subject, topic, data) {
      ...
      switch (data) {
        case "plugin.disable":
          torbutton_toggle_plugins(m_tb_prefs.getBoolPref("plugin.disable"));
          break;
  ...
  var torbutton_abouttor_message_handler = {
    ...
    // not working.
    getChromeData(aIsRespondingToPageLoad) {
      let dataObj = {
  ...
  function torbutton_confirm_plugins() {
    ...
    var message = torbutton_get_property_string(
      "torbutton.popup.confirm_plugins"
    );
    var askAgainText = torbutton_get_property_string(
    ...
    if (no_plugins != m_tb_prefs.getBoolPref("plugin.disable")) {
  ...
  function torbutton_send_ctrl_cmd(command) {
    ...
    try {
      ...
      var inputStream = Cc["@mozilla.org/binaryinputstream;1"].createInstance(
        Ci.nsIBinaryInputStream
      );
      var outputStream = Cc["@mozilla.org/binaryoutputstream;1"].createInstance(
      ...
      if (bytes.indexOf("250") != 0) {
        torbutton_safelog(
          4,
          "Unexpected auth response on control port " + m_tb_control_desc + ":",
          bytes
        );
        return null;
      }
      ...
      if (bytes.indexOf("250") != 0) {
        torbutton_safelog(
          4,
          "Unexpected command response on control port " +
            m_tb_control_desc +
            ":",
          bytes
        );
  ...
  torbutton_new_identity = async function() {
    try {
      ...
      let shouldConfirm = m_tb_prefs.getBoolPref(
      ...
      if (shouldConfirm) {
        ...
        let askAgainText = torbutton_get_property_string(
          "torbutton.popup.never_ask_again"
        );
  ...
  async function torbutton_do_new_identity() {
    ...
    torbutton_log(
      3,
      "New Identity: Emitting Private Browsing Session clear event"
    );
    ...
    if (m_tb_prefs.getBoolPref("extensions.torbutton.clear_http_auth")) {
      var auth = Cc["@mozilla.org/network/http-auth-manager;1"].getService(
    ...
    try {
      ...
      for (let contextInfo of [
        LoadContextInfo.default,
        LoadContextInfo.private,
      ]) {
        let appCacheStorage = Services.cache2.appCacheStorage(
          contextInfo,
          null
        );
      ...
      window.alert(
        "Torbutton: Unexpected error during offline cache clearing: " + e
      );
    }
  ...
  function torbutton_do_tor_check() {
    if (
      checkSvc.kCheckNotInitiated != checkSvc.statusOfTorCheck ||
